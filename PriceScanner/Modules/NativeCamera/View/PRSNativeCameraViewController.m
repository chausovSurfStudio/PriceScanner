...
@property (nonatomic, strong) NSDate *startSymbolDate;

@property (nonatomic, strong) NSMutableArray<NSNumber *> *rectangleDetectedTimes;
...
- (void)viewDidLoad {
    ...
    self.rectangleDetectedTimes = [@[] mutableCopy];
...
- (void)handleTextDetectRequest:(VNRequest *)request {
    if (self.scanTimer.state == PRSScanTimerStateSnapshot) {
        ...
        NSTimeInterval interval = [date timeIntervalSinceDate:self.startSymbolDate];
...
- (void)completeTextScanWithResultConfidence:(CGFloat)confidence {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (confidence >= 0.99f) {
            ...
            NSLog(@"--------------------------------------------------------------");
            [self printTimes:self.rectangleDetectedTimes withTitle:@"Время распознавания прямоугольника"];
...
- (void)handleRectangleRequest:(VNRequest *)request {
    ...
    if (self.scanTimer.state == PRSScanTimerStateSnapshot) {
        NSLog(@"------------------------------------------------------------------");
        NSLog(@"прямоугольник определить %@", rectangle == nil ? @"не удалось" : @"удалось");
    }
    if (!rectangle) {
        return;
    }
    ...
    if (self.scanTimer.state == PRSScanTimerStateSnapshot) {
        ...
        NSLog(@"время определения прямоугольника = %f", interval);
        [self.rectangleDetectedTimes addObject:@(interval)];
        self.startSymbolDate = [NSDate date];
    }
    dispatch_async(dispatch_get_main_queue(), ^{
